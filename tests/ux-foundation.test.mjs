...
import {
  ...
  shouldMountCursor,
  rsvpContext,
  clearRsvpSelectionOnContextChange,
...
test('surface boundaries cover nested workspaces, access, alias and guest routes without substring matches', () => {
  for (const [path, surface, workspace] of [
    ['/', 'marketing'],
    ['/services/rsvp', 'marketing'],
    ['/clientele', 'marketing'],
    ['/login?workspace=client', 'access'],
    ['/rsvp/login', 'access', 'rsvp'],
    ['/admin', 'workspace', 'operations'],
    ['/operations/reviews', 'workspace', 'operations'],
    ['/rsvp/events/event-a', 'workspace', 'rsvp'],
    ['/rsvp/invite/token-a', 'guest-invitation'],
  ]) {
    assert.equal(routeInfo(path).surface, surface);
    assert.equal(routeInfo(path).workspace, workspace);
  }
});
test('parser rejects untrusted shape, mismatched workspace, unknown IDs, injected claims and versions', () => {
  for (const raw of [
    ...
    assert.equal(parseDemoSession(raw), null);
...
test('explicit selection persists only fixture identifiers and keeps records and other tabs intact', () => {
  const disk = storage();
  disk.setItem('preview-records', 'keep');
...
test('read, write and removal failures retain latest memory selection, including exit', () => {
  ...
  const denied = createDemoSessionStore(() => {
    throw new Error('security');
  });
  denied.read();
  denied.select('operations-reviewer');
...
test('RSVP adapter separates guest and staff context and invalidates local selection on event or organization switch', () => {
  ...
  assert.deepEqual(rsvpContext('/rsvp/events/a/inbox'), {
    kind: 'event',
    eventId: 'a',
  });

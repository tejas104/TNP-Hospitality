...
import type {
  Quote, Earning, Payout, Collection, Paginated, QueryOptions,
  AnyMutationRequest,
  Application,
  Assignment,
  Attendance,
...
export function AdminOperations() {
  ...
  const refreshAll = useCallback(async (retainVisible = false) => {
    ...
    if (!retainVisible) setLoadState({ kind: 'loading', message: 'Refreshing linked Operations records…' });
  ...
  const beginOperation = useCallback(async <Operation extends PreviewOperation>(
    operation: Operation, payload: PreviewMutationMap[Operation]['payload'], label: string, actorId = 'tnp-demo-ops-001',
  ) => {
    const instance = service.current;
    if (!instance) return;
    ...
    const request: MutationRequest<Operation> = {
      ...
      expectedGeneration: await instance.getGeneration(), actorId, operation, payload,
    };
    if (!isCurrentActionEpoch(epoch, actionEpoch.current)) return;
      ...
      <section className="admin-main">
        ...
        {loadState.kind === 'ready' ? <>
          ...
          <section className={styles.workspace} id="operations-workspace" ref={workspaceRef} tabIndex={-1} aria-label={`${sections.find((item) => item.current)?.label} workspace`}>
            {panel === 'finance' && <FinancePanel key={data.generation} refreshSignal={feedback.kind+feedback.actionId} feedback={feedback} canRetry={feedbackCanRetry} onRetry={retryCurrentAction} onApprove={(earningId,stage)=>void beginOperation('approveEarning',{earningId,stage},'Sample '+stage+' approval',stage==='supervisor'?'tnp-demo-supervisor-001':'tnp-demo-finance-001')} onAdjust={payload=>void beginOperation('adjustEarning',payload,'Sample earning adjustment','tnp-demo-finance-001')} onRevise={payload=>void beginOperation('reviseQuote',payload,'Sample quote revision','tnp-demo-finance-001')}/>}
...
function OverviewPanel({ events, positions, requirements, onSelectEvent }: { events: PreviewEvent[]; positions: Position[]; requirements: Requirement[]; onSelectEvent: (id: string) => void }) {
  return <><div className={styles.panelHeading}><div><p className="section-kicker">OPERATIONS OVERVIEW</p><h2>Review event staffing and outstanding decisions</h2></div><p>Totals come from the shared synthetic scenario, not a local Operations fixture.</p></div><div className="admin-grid">{events.map((event) => {
...
function FinancePanel({
  ...
  return (
    <div className={styles.finance}>
      {quote&&tab==='Quotes'&&<article className={styles.printQuote} data-sample-quote><header><p>SAMPLE — NOT AN OFFICIAL QUOTATION OR INVOICE</p><h1>TNP · Sample quotation</h1><p>{quote.issuingEntity}</p></header><p>Reference {quote.id} · Version {quote.version} · {quote.status}</p><p>Booking {quote.bookingId}</p><table><thead><tr><th>Description</th><th>Quantity</th><th>Unit rate</th><th>Total</th></tr></thead><tbody>{quote.lines.map(line=><tr key={line.id}><td>{line.label}</td><td>{line.quantity}</td><td>{money(line.unitPaise)}</td><td>{money(line.totalPaise)}</td></tr>)}</tbody></table><h2>Total {money(quote.totalPaise)}</h2><p>{quote.revisionReason}</p><footer>Browser preview only. This document does not issue a commercial offer, invoice, tax receipt or payment acknowledgement.</footer></article>}

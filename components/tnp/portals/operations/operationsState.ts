...
export const OPERATIONS_SECTIONS: ReadonlyArray<{
  ...
  { id: 'finance', label: 'Quotes & earnings' },
  { id: 'catalogue', label: 'Catalogue & access' },
];

// Reports & audit is a working, read-only section promoted from the later-milestone list.
export const REPORTS_SECTION: { id: OperationsSection; label: string } = {
  id: 'reports',
  label: 'Reports & audit',
};

export function operationsSectionItems(current: OperationsSection) {
  return [...OPERATIONS_SECTIONS, REPORTS_SECTION].map((section) => ({
    ...section,
...
export function filterOperationsEvents(
  ...
  return events.filter((event) => {
    const matchesTerm =
      !term ||
      `${event.name} ${event.id} ${event.status}`.toLowerCase().includes(term);
    return (
      matchesTerm && (statusFilter === 'all' || event.status === statusFilter)
    );
  });
}
...
export function actionableAttendanceAssignments(
  assignments: Assignment[],
  attendances: Attendance[],
) {
  const recordedAssignmentIds = new Set(
    attendances.map((attendance) => attendance.assignmentId),
  );
...
export function canRetryFeedback(
  feedback: RetryIdentity,
  retainedActionId: string | undefined,
) {
  return Boolean(

...
const subscribeMotion = (callback: () => void) =>
  subscribeQuery(motionQuery, callback);
const subscribeMobile = (callback: () => void) =>
  subscribeQuery(mobileQuery, callback);
...
class SceneBoundary extends Component<
  { children: ReactNode; onFailure: () => void; fallback: ReactNode },
  { failed: boolean }
> {
  state = { failed: false };
...
export default function HomeHero({
  ...
  const handleFailure = useCallback(() => {
    ...
    setPresentation({
      service: serviceAt(current.angle),
      turning: false,
      angle: current.angle,
    });
    setFailed(true);
  }, []);
  const policy = cubePolicy({
    reduced,
    mobile,
    ...capability,
    failed,
  ...
  const publish = useCallback((force = false) => {
    ...
    if (container.current)
      container.current.dataset.cubeAngle = current.angle.toFixed(4);
    setPresentation((previous) =>
      !force && previous.service === service && previous.turning === turning
        ? previous
        : { service, turning, angle: current.angle },
    );
  }, []);
  useEffect(() => subscribeQuery(motionQuery, () => publish(true)), [publish]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (!policy.animate || !ready) return;
    let frame = 0;
  ...
  const move = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (current.intent === 'pending') {
      ...
      if (current.intent === 'horizontal') {
        ...
        motion.current = beginDrag(motion.current);
        setDragging(true);
      }
    }
    if (current.intent !== 'horizontal') return;
    motion.current = dragTo(
      motion.current,
      current.angle,
      dx,
  ...
  const end = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (gesture.current.intent === 'horizontal')
      motion.current = endDrag(motion.current);
  ...
  const fallback = <HospitalityCubeFallback angle={presentation.angle} />;
  const still = policy.tier === 'static';
  return (
    <div
      ref={container}
      className={`${styles.heroArt} ${cube.art}`}
      data-hero-art
      data-cube-tier={policy.tier}
      data-cube-angle={presentation.angle.toFixed(4)}
    >
      <div
      ...
      <div
        ...
        aria-hidden="true"
        data-scene-state={
          still ? 'still' : policy.animate ? 'running' : 'suspended'
        }
      ...
      >
        {visited && !still ? (
          <SceneBoundary onFailure={handleFailure} fallback={fallback}>
            <Suspense fallback={fallback}>
              <PavilionScene
                active={policy.animate}
                fallback={fallback}
                onFailure={handleFailure}
                onReady={handleReady}
                motion={motion}
                clock={clock}
                mobile={policy.tier === 'mobile'}
                revision={presentation.angle}
              />
            </Suspense>
          </SceneBoundary>
        ) : (

...
import type { MembershipDocument, OrganizationDocument, UserDocument } from "../tenancy/model.ts";
...
export interface SessionRepository {
  ...
  findUserById(userId: string, transaction?: RepositoryTransaction): Promise<UserDocument | null>;
  findMembershipById(
    organizationId: string,
    membershipId: string,
    transaction?: RepositoryTransaction,
  ): Promise<MembershipDocument | null>;
  insertSession(session: SessionDocument, transaction?: RepositoryTransaction): Promise<void>;
  revokeSession(
    organizationId: string,
    sessionId: string,
    revokedBy: string,
...
export function readSessionToken(request: Request, cookieName: string): string {
  ...
  if (!/^[A-Za-z0-9_-]{43}$/.test(token)) throw new SessionError("SESSION_INVALID", 401);
  return token;
}
...
export async function authenticateRequest(
  ...
  const session = await repository.findSessionByTokenHash(digest(environment.sessionSecret, token));
  if (!session) throw new SessionError("SESSION_INVALID", 401);
  if (session.revokedAt) throw new SessionError("SESSION_REVOKED", 401);
  if (session.expiresAt.getTime() <= now.getTime()) throw new SessionError("SESSION_EXPIRED", 401);
  ...
  if (!user || user.status !== "active") throw new SessionError("IDENTITY_INACTIVE", 403);
  if (
    !membership ||
...
export function requireCsrf(
  ...
  if (!token || token.length > 256) throw new SessionError("CSRF_REJECTED", 403);
...
export function issueSession(
  ...
  const maxAge = Math.max(0, Math.floor((input.expiresAt.getTime() - input.createdAt.getTime()) / 1000));

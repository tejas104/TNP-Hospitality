...
export class ConfigurationError extends Error {
  ...
  constructor(readonly issues: readonly ConfigurationIssue[]) {
    super(`Required platform configuration is unavailable: ${issues.map((issue) => issue.name).join(", ")}`);
...
export function loadPlatformEnvironment(
  ...
  if (sessionSecret.length < 32) issues.push({ name: "SESSION_SECRET", reason: "invalid" });
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(appBaseUrl.protocol) || appBaseUrl.username || appBaseUrl.password) {

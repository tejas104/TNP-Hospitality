...
The user requires a separately sellable TNP-managed RSVP, guest hospitality and logistics service with a dedicated customer login/portal. Customers oversee their own events while TNP operates the service. Include function-wise guest responses, independent travel/pickup/stay/document classifications, guest forms, WhatsApp-connected documents, rooming, transport and controlled Excel/PDF reporting. The supplied T-30 to post-event brief is the operating requirements source. See [RSVP service blueprint](RSVP-SERVICE-BLUEPRINT.md) for the proposed architecture and delivery batches. Commercial packages, detailed document policy, provider selection and delivery reforecast remain pending; this is not a new builder lease or production activation.
...
The new user instruction expands the previous basic RSVP baseline to a TNP-managed service; it does not authorize silently deferring WhatsApp or documents. Do not expose live guest-document collection until the secure lifecycle and access rules are implemented and accepted. Additional functionality needs a delivery reforecast rather than being silently folded into existing lane milestones.

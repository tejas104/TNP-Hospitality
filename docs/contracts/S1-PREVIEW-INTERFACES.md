...
Storage key/prefix is fixed: tnp-preview-v1. The stored envelope includes a numeric generation, records and a request-result ledger. Reset increments generation, replaces records with the deterministic initial scenario and empties the ledger in the same persisted update. Refresh preserves the current generation and ledger. Mutation request identity is (generation, requestKey). Reusing the same key after Reset executes once in the new generation; retrying it within that generation replays its first result. Stale pre-reset in-flight responses cannot modify the new generation. Capture an explicit deterministic clock/timezone; do not depend on wall-clock dates making scenarios expire unexpectedly.
...
Every operation is Promise-based with typed success/error outcomes. Every mutation takes a requestKey and actor/scenario context. Same key + same payload returns the original result without another effect; same key + different payload rejects with IDEMPOTENCY_CONFLICT. Reset keeps the storage key and starts a new generation with an empty request ledger, as defined above; an old-generation request is rejected as STALE_GENERATION, not silently applied. Query failures and deterministic mutation failures must be reproducible.

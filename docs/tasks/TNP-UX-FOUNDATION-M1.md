...
Status: BUILDING — exclusive writer lease active. LAUNCH_SHA is exact `1c2b8d570a7d62f52c01710f190bdb054fdbf632`. P created `codex/tnp-ux-foundation-m1` at `D:\TNP-worktrees\TNP-UX-FOUNDATION-M1`, then author task `01a0bee6-65ab-73c2-a0c6-60eb83111347` independently proved clean exact HEAD, SOURCE and dependency ancestry, the three-file launch delta and free port3118 before starting source work. The baseline has no RSVP pages, so this milestone owns only an explicit typed future-route/unavailable adapter boundary; it must not link users to nonexistent RSVP pages or add frozen RSVP feature files.

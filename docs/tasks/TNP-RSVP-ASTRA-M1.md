...
Status: TECHNICAL ROUND-6 PASS PRESERVED — PRODUCT ACCEPTANCE/INTEGRATION PAUSED BY 2026-09-19 SCOPE CORRECTION. Clean pushed remote-equal final `0c562341cb97b1a3e1ecc6638677850a6692552f` passed its then-current independent Sol/high technical review. The user subsequently removed phone calling, room/vehicle allocation, dispatch and booking/payment behavior from RSVP and required WhatsApp-only information collection plus organization-level daily multi-event and isolated per-event workspaces. Do not seek Kartik product acceptance or integrate this SHA unchanged. Preserve it as predecessor evidence for the bounded `TNP-RSVP-MESSAGE-ONLY-M2` correction; no lease is reopened by this note.
...
2. Clearing current arrival/departure details must release/invalidate active dependent movement plans, but it must not orphan or erase immutable historical movement evidence. `guest-met` and `completed` transfers must retain a valid referenced leg snapshot, vehicle and plan basis and remain visible in historical manifests/audit views. Delete a leg only when no retained transfer references it. Assignment/dispatch for current/future movement remains blocked until details and replan. Fixed-SHA review identified that existing `logic.ts` excludes completed transfers from manifests even before clearing. The Ready scope therefore permits a minimal explicit history/guard mode in `buildManifests`: normal display includes completed history, while adapter capacity/overlap guard calls explicitly exclude completed transfers so historical work cannot block new allocation. Add focused proof for both modes. No other logic expansion is allowed.

Required regressions: construct a real retained Round-3-style duplicate receipt batch and prove exact accepted/rejected replay plus moved/unseen conflict; clear travel with requested/planned/assigned plus guest-met/completed dependents and prove active release, blocked future dispatch, no dangling `legId`, preserved historical manifest/vehicle/plan evidence and no accidental deletion; prove completed history appears in display manifests but is excluded from active vehicle capacity/overlap enforcement. Writable Round-5 source is only `adapter.ts`, the minimal history/guard mode in `logic.ts`, and focused RSVP-local tests/browser evidence. No UI/CSS/design changes.

...
| **6–8 — Sep 23–25** | **Astra frontend, serialized by ownership:** corrected RSVP message-only multi-event UI plus Freelancer application/assessment/opportunity/Coming UI in separate Ready leases; Client UI follow-up where production contracts require it. **H2 platform/domain:** persistent RSVP/workforce services, authorization, isolation, message jobs, attendance and finance contracts. RSVP calling, booking, room/vehicle allocation, dispatch and payment UI are excluded. Provider-disabled states stay truthful. | Two-vendor and cross-event isolation probes; import/reply replay and ambiguity tests; WhatsApp delivery/retry states; capacity/eligibility/overlap tests; attendance evidence; payout/earning derivation; responsive/keyboard flows. |
